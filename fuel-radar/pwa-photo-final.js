...
function labels(){
 ...
 const input=$('mediaFile');if(input){if(input.accept!=='image/*')input.accept='image/*';input.removeAttribute('capture')}
 ...
 const label=document.querySelector('label[for="mediaFile"]');if(label&&label.textContent!=='Выберите фото или снимите камерой')label.textContent='Выберите фото или снимите камерой';
...
function editor(){
 ...
 wrap.innerHTML='<div class="rpf-card"><div class="rpf-head"><div><b>🔒 Проверка фото</b><span id="rpfStatus">Подготавливаю фото…</span></div><button id="rpfClose" type="button">✕</button></div><div class="rpf-tip">Проверьте лица и госномера. Чтобы скрыть область, проведите по ней пальцем или мышью.</div><div class="rpf-stage"><canvas id="rpfCanvas"></canvas></div><div class="rpf-actions"><button id="rpfUndo" type="button">↩ Отменить область</button><button id="rpfDone" class="rpf-ok" type="button">✓ Готово, использовать фото</button></div></div>';
 const st=document.createElement('style');st.textContent='#radarPwaPhotoEditorFinal{position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,.86);padding:max(12px,env(safe-area-inset-top)) 12px max(12px,env(safe-area-inset-bottom));overflow:auto;color:#111827;font:15px/1.4 system-ui,-apple-system,Segoe UI,sans-serif}#radarPwaPhotoEditorFinal[hidden]{display:none}.rpf-card{max-width:760px;margin:auto;background:#fff;border-radius:18px;padding:14px;box-shadow:0 24px 80px rgba(0,0,0,.45)}.rpf-head{display:flex;justify-content:space-between;gap:12px}.rpf-head b{font-size:19px}.rpf-head span{display:block;font-size:12px;color:#64748b;margin-top:3px}.rpf-head button,.rpf-actions button{min-height:44px;border:1px solid #d7dce3;border-radius:12px;background:#fff;color:#111827;font-weight:800;padding:0 13px}.rpf-tip{margin:12px 0;padding:10px 12px;border-radius:12px;background:#eff6ff;color:#1e3a8a;font-size:13px}.rpf-stage{background:#111827;border-radius:14px;overflow:hidden;display:grid;place-items:center;touch-action:none}.rpf-stage canvas{display:block;max-width:100%;max-height:68vh;touch-action:none}.rpf-actions{display:grid;grid-template-columns:1fr 1.4fr;gap:9px;margin-top:12px}.rpf-actions .rpf-ok{background:#111827;color:#fff}@media(max-width:560px){.rpf-actions{grid-template-columns:1fr}.rpf-stage canvas{max-height:60vh}}';
 document.head.appendChild(st);document.body.appendChild(wrap);editorCanvas=$('rpfCanvas');
 $('rpfClose').onclick=()=>{wrap.hidden=true;sourceCanvas=null;rects=[];drag=null};$('rpfUndo').onclick=()=>{rects.pop();draw()};$('rpfDone').onclick=finish;
...
function pixelate(ctx,base,r){const x=Math.max(0,Math.floor(r.x)),y=Math.max(0,Math.floor(r.y)),w=Math.min(base.width-x,Math.ceil(r.w)),h=Math.min(base.height-y,Math.ceil(r.h));if(w<2||h<2)return;const mini=document.createElement('canvas');mini.width=Math.max(1,Math.round(w/14));mini.height=Math.max(1,Math.round(h/14));mini.getContext('2d').drawImage(base,x,y,w,h,0,0,mini.width,mini.height);ctx.save();ctx.imageSmoothingEnabled=false;ctx.drawImage(mini,0,0,mini.width,mini.height,x,y,w,h);ctx.restore()}
function draw(){if(!sourceCanvas||!editorCanvas)return;editorCanvas.width=sourceCanvas.width;editorCanvas.height=sourceCanvas.height;const c=editorCanvas.getContext('2d');c.drawImage(sourceCanvas,0,0);rects.forEach(r=>pixelate(c,sourceCanvas,r))}
