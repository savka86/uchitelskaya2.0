...
export function TransitAppRoadSegments({ initialData }: { initialData: ScheduleData }) {
      ...
      try {
        ...
        for (const routeStop of forwardStops) {
          ...
          if (!Number.isFinite(stop?.latitude) || !Number.isFinite(stop?.longitude)) {
            throw new Error(`Нет точных координат остановки «${stop?.name ?? routeStop.stop_id}».`);
          }
          const coordinate: Coordinates = [Number(stop?.latitude), Number(stop?.longitude)];
        ...
        const busLayout = ymaps.templateLayoutFactory.createClass(
          `<div style="display:flex;align-items:center;gap:6px;transform:translate(-26px,-26px);white-space:nowrap;">
            <div style="display:grid;width:52px;height:52px;place-items:center;border:4px solid #fff;border-radius:16px;background:#c9f04a;box-shadow:0 8px 24px rgba(24,33,50,.42);font-size:29px;">🚌</div>
            <b style="padding:7px 10px;border-radius:9px;background:#182132;color:#fff;font:900 12px Arial,sans-serif;box-shadow:0 4px 12px rgba(24,33,50,.2);">АВТОБУС № ${initialData.route?.route_number ?? "2"}</b>
          </div>`,
        );
        const bus = new ymaps.Placemark(coordinates[0], {
          ...
          iconShape: { type: "Circle", coordinates: [0, 0], radius: 32 },
          zIndex: 50000,
          zIndexHover: 60000,
        });
        ...
        setMapReady(true);
        const stopBounds = map.geoObjects.getBounds?.();
        if (stopBounds) map.setBounds(stopBounds, { checkZoomRange: true, zoomMargin: 48 });
        ...
        for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex += 1) {
          ...
          setMapMessage(`Автобус показан · строю дороги ${chunkIndex + 1}/2…`);
          const chunkCoordinates = coordinates.slice(chunk.start, chunk.end + 1);
          const route = await ymaps.route(chunkCoordinates, {
            multiRoute: false,
            routingMode: "auto",
          ...
          for (let localIndex = 0; localIndex < paths.getLength() && localIndex < expected; localIndex += 1) {
            const path = paths.get(localIndex);
            ...
            const from = forwardStops[globalIndex].stop_id;
            const to = forwardStops[globalIndex + 1].stop_id;
        ...
        const allBounds = map.geoObjects.getBounds?.();
        if (allBounds) map.setBounds(allBounds, { checkZoomRange: true, zoomMargin: 48 });
        if (completedSegments === 13) {
          setMapMessage("Готово · 13/13 участков по дорогам Яндекса · автобус движется по дороге");
        } else {
          setMapMessage(`Построено ${completedSegments}/13 дорожных участков · автобус остаётся на последней подтверждённой точке`);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (road) {
      ...
      const safeStop = stopCoordsRef.current.get(live.previous.stopId) ?? stopCoordsRef.current.get(live.selectedTrip.events[0]?.stopId);
      if (safeStop) busPlacemarkRef.current.geometry?.setCoordinates?.(safeStop);
    }
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace" id="top">
        <aside className="sidebar">
          <div className="intro">
            ...
            <p>Автобус движется расчётно по расписанию строго по дорожной геометрии Яндекса.</p>
          </div>
        ...
        <section className="map-panel" aria-label={`Яндекс Карта маршрута № ${initialData.route.route_number}`}>
          <div className="map-toolbar">
            ...
            <span className="map-mode">ЯНДЕКС · ДОРОГИ 1–8 / 8–14</span>
          </div>
          ...
          <div className="map-canvas">
            ...
            <div style={{ position: "absolute", right: 14, top: 14, zIndex: 20, display: "flex", gap: 8 }}>
              <button type="button" onClick={() => {
                ...
                if (bounds) yandexMapRef.current?.setBounds?.(bounds, { checkZoomRange: true, zoomMargin: 48 });
      ...
      <section className="schedule-section">
        ...
        <p className="schedule-note">Все 14 остановок берутся по точным координатам из Supabase. Для устойчивости дорожный маршрут строится двумя частями: 1–8 и 8–14; автобус движется по геометрии соответствующего дорожного участка.</p>
      </section>

      <footer className="data-footer">
        <span>ЯНДЕКС ДОРОГИ · SUPABASE</span>
        <strong>{initialData.route.name}</strong>
        <p>Маршрут № {initialData.route.route_number} · автобус по дорогам · расчёт по расписанию.</p>
      </footer>
    </main>
  );
}
